    ...
    return (
      <div className="w-full">
        ...
        {results.suggestions && results.suggestions.length > 0 && (
          <div className="mt-8">
            <h3 className="text-xl font-semibold mb-4 text-gray-800">You might also be interested in:</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {results.suggestions.map((suggestion, index) => (
                <div
                  key={index}
                  className="bg-white border border-sky-100 rounded-lg p-4 hover:border-teal-200 transition-colors cursor-pointer"
                >
                  <p className="font-medium text-teal-700">{suggestion}</p>
                </div>
              ))}
            </div>
          </div>
